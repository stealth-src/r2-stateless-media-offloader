...
?>
<div class="wrap">
	<h1><?php esc_html_e( 'Migrate Media to R2', 'r2-stateless-media-offloader' ); ?></h1>

	<p class="description" style="max-width:46em;">
		<?php esc_html_e( 'Copies existing media (original + every size) into R2, reading from a local copy when present or fetching from the current public URL otherwise — so it works whether your media is local, on GCS, or on S3. Runs in the background and survives leaving this page.', 'r2-stateless-media-offloader' ); ?>
	</p>
	<p class="description" style="max-width:46em;">
		<?php
		echo wp_kses_post(
			__( 'For very large libraries, the WP-CLI command is recommended: <code>wp r2offload sync</code>.', 'r2-stateless-media-offloader' )
		);
		?>
	</p>
	<p class="description" style="max-width:46em;">
		<?php
		echo wp_kses_post(
			__( '<strong>Already copied your media into R2 another way</strong> (e.g. Cloudflare R2 data migration / Super Slurper)? Just run Migrate — files already in R2 are detected and registered without re-uploading.', 'r2-stateless-media-offloader' )
		);
		?>
	</p>
	...
	<?php if ( ! $r2offload_configured ) : ?>
		<div class="notice notice-warning"><p>
			...
			printf(
				...
				wp_kses_post( __( 'R2 is not configured yet. <a href="%s">Add your credentials</a> first.', 'r2-stateless-media-offloader' ) ),
	...
	<table class="form-table" role="presentation">
		<tr>
			<th scope="row"><label for="r2offload-mig-mode"><?php esc_html_e( 'Mode', 'r2-stateless-media-offloader' ); ?></label></th>
			<td>
				<?php $r2offload_mode = isset( $state['mode'] ) ? (string) $state['mode'] : 'upload'; ?>
				<select id="r2offload-mig-mode" <?php disabled( $r2offload_has_run ); ?>>
					<option value="upload" <?php selected( $r2offload_mode, 'upload' ); ?>><?php esc_html_e( 'Migrate (upload to R2)', 'r2-stateless-media-offloader' ); ?></option>
					<option value="force" <?php selected( $r2offload_mode, 'force' ); ?>><?php esc_html_e( 'Force re-upload (replace existing in R2)', 'r2-stateless-media-offloader' ); ?></option>
					<option value="dry-run" <?php selected( $r2offload_mode, 'dry-run' ); ?>><?php esc_html_e( 'Dry run (count + size, no upload)', 'r2-stateless-media-offloader' ); ?></option>
					<option value="verify" <?php selected( $r2offload_mode, 'verify' ); ?>><?php esc_html_e( 'Verify (check objects exist in R2)', 'r2-stateless-media-offloader' ); ?></option>
				</select>
			</td>
		</tr>
	</table>
	...
	$r2offload_pause_lbl  = $r2offload_can_resume
		? __( 'Resume', 'r2-stateless-media-offloader' )
		: __( 'Pause', 'r2-stateless-media-offloader' );
	?>
	<p>
		...
		<button type="button" class="button button-primary" id="r2offload-mig-start" <?php disabled( $r2offload_has_run ); ?>>
			<?php esc_html_e( 'Start', 'r2-stateless-media-offloader' ); ?>
		</button>
		...
		<button type="button" class="button" id="r2offload-mig-stop" <?php disabled( ! $r2offload_has_run ); ?>>
			<?php esc_html_e( 'Stop', 'r2-stateless-media-offloader' ); ?>
		</button>
	</p>
	...
	<div style="max-width:46em;">
		...
		<p id="r2offload-mig-text" aria-live="polite" style="margin-top:.5em;display:flex;align-items:center;gap:4px;">
			...
			<span id="r2offload-mig-text-inner"><?php esc_html_e( 'Idle', 'r2-stateless-media-offloader' ); ?></span>
		</p>
		...
		<details id="r2offload-mig-log-details" style="margin-top:1em;">
			<summary style="cursor:pointer;"><?php esc_html_e( 'Activity log', 'r2-stateless-media-offloader' ); ?></summary>
			<div id="r2offload-mig-log"
				...
				aria-label="<?php esc_attr_e( 'Migration activity log', 'r2-stateless-media-offloader' ); ?>"
			></div>
		</details>

		<details style="margin-top:1em;">
			<summary style="cursor:pointer;"><?php esc_html_e( 'What do these counts mean?', 'r2-stateless-media-offloader' ); ?></summary>
			<table class="widefat striped" style="max-width:46em;margin-top:.5em;">
				<tbody>
					<tr>
						<td style="width:8em;"><strong><?php esc_html_e( 'Uploaded', 'r2-stateless-media-offloader' ); ?></strong></td>
						<td><?php esc_html_e( 'The file was not in R2 — it was newly copied up.', 'r2-stateless-media-offloader' ); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e( 'Updated', 'r2-stateless-media-offloader' ); ?></strong></td>
						<td><?php esc_html_e( 'The file was already in R2 but the wrong size (or you chose Force re-upload), so it was replaced.', 'r2-stateless-media-offloader' ); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e( 'Adopted', 'r2-stateless-media-offloader' ); ?></strong></td>
						<td><?php esc_html_e( 'The correct file was already in R2 (e.g. copied by Cloudflare Super Slurper). No bytes were moved — WordPress was just registered to serve it from R2.', 'r2-stateless-media-offloader' ); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e( 'Skipped', 'r2-stateless-media-offloader' ); ?></strong></td>
						<td><?php esc_html_e( 'Already in R2 and already registered by a previous run — nothing to do.', 'r2-stateless-media-offloader' ); ?></td>
					</tr>
				</tbody>
			</table>
		</details>
		...
		<details style="margin-top:1em;">
			<summary style="cursor:pointer;"><?php esc_html_e( 'How does the background job work?', 'r2-stateless-media-offloader' ); ?></summary>
			<div style="max-width:46em;margin-top:.5em;">
				...
				<table class="widefat striped" style="margin-top:.5em;">
					<tbody>
						<tr>
							<td style="width:6em;"><strong><?php esc_html_e( 'Pause', 'r2-stateless-media-offloader' ); ?></strong></td>
							<td><?php esc_html_e( 'Stops processing after the current batch finishes and saves your position. Click Resume to continue from exactly where you left off — no work is lost.', 'r2-stateless-media-offloader' ); ?></td>
						</tr>
						<tr>
							<td><strong><?php esc_html_e( 'Stop', 'r2-stateless-media-offloader' ); ?></strong></td>
							<td><?php esc_html_e( 'Cancels the run and discards its position and counters — files already migrated to R2 stay in R2 and stay registered. The next Start scans the library from the beginning; already-migrated items are detected and skipped quickly.', 'r2-stateless-media-offloader' ); ?></td>
						</tr>
					</tbody>
				</table>
			</div>
